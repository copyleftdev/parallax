...
fn build_stats(engine: &StorageEngine) -> IndexStats {
    ...
    for e in &all {
        *type_counts.entry(e._type.as_str().to_owned()).or_insert(0usize) += 1;
        *class_counts.entry(e._class.as_str().to_owned()).or_insert(0usize) += 1;
    }
    IndexStats::new(type_counts, class_counts, snap.entity_count(), snap.relationship_count())
}
...
fn pql_find_with_property_filter() {
    ...
    let mut batch = WriteBatch::new();
    batch.upsert_entity(entity_with_props("host", "Host", "h1", &[("state", Value::from("running"))]));
    batch.upsert_entity(entity_with_props("host", "Host", "h2", &[("state", Value::from("stopped"))]));
    engine.write(batch).unwrap();
...
fn pql_or_filter_returns_both_matching() {
    ...
    let mut batch = WriteBatch::new();
    batch.upsert_entity(entity_with_props("host", "Host", "linux-h",   &[("os", Value::from("linux"))]));
    batch.upsert_entity(entity_with_props("host", "Host", "windows-h", &[("os", Value::from("windows"))]));
    batch.upsert_entity(entity_with_props("host", "Host", "macos-h",   &[("os", Value::from("macos"))]));
    engine.write(batch).unwrap();

    let count = pql_count(&engine, "FIND host WITH os = 'linux' OR os = 'windows'");
    assert_eq!(count, 2, "OR filter must return linux AND windows hosts, not macos");
}
...
fn pql_or_single_arm_same_as_eq() {
    ...
    let mut batch = WriteBatch::new();
    batch.upsert_entity(entity_with_props("host", "Host", "h1", &[("env", Value::from("prod"))]));
    batch.upsert_entity(entity_with_props("host", "Host", "h2", &[("env", Value::from("dev"))]));
    engine.write(batch).unwrap();

    let eq_count  = pql_count(&engine, "FIND host WITH env = 'prod'");
    let or_count  = pql_count(&engine, "FIND host WITH env = 'prod' OR env = 'prod'");
    assert_eq!(eq_count, or_count, "single-armed OR must match plain equality");
...
fn pql_or_combined_with_and_property() {
    ...
    batch.upsert_entity(entity_with_props("host", "Host", "match",    &[("os", Value::from("linux")),   ("env", Value::from("prod"))]));
    // OS matches OR but env doesn't → no match
    batch.upsert_entity(entity_with_props("host", "Host", "no-env",   &[("os", Value::from("windows")), ("env", Value::from("dev"))]));
    // env matches but OS doesn't → no match
    batch.upsert_entity(entity_with_props("host", "Host", "no-os",    &[("os", Value::from("macos")),   ("env", Value::from("prod"))]));
    engine.write(batch).unwrap();

    let count = pql_count(&engine, "FIND host WITH os = 'linux' OR os = 'windows' AND env = 'prod'");
...
fn pql_or_three_alternatives() {
    ...
    for region in ["us-east-1", "eu-west-1", "ap-southeast-1", "sa-east-1"] {
        batch.upsert_entity(entity_with_props("host", "Host", region, &[("region", Value::from(region))]));
    }
    engine.write(batch).unwrap();

    let count = pql_count(&engine, "FIND host WITH region = 'us-east-1' OR region = 'eu-west-1' OR region = 'ap-southeast-1'");
...
fn pql_group_by_correct_groups() {
    ...
    let mut batch = WriteBatch::new();
    batch.upsert_entity(entity_with_props("host", "Host", "h1", &[("os", Value::from("linux"))]));
    batch.upsert_entity(entity_with_props("host", "Host", "h2", &[("os", Value::from("linux"))]));
    batch.upsert_entity(entity_with_props("host", "Host", "h3", &[("os", Value::from("windows"))]));
    engine.write(batch).unwrap();
    ...
    let QueryResult::Grouped(groups) = result else { panic!("expected Grouped") };
    assert_eq!(groups.len(), 2, "two distinct OS values → two groups");

    let linux_count   = groups.iter().find(|(v, _)| matches!(v, Value::String(s) if s.as_str() == "linux"))
        .map(|(_, c)| *c);
    let windows_count = groups.iter().find(|(v, _)| matches!(v, Value::String(s) if s.as_str() == "windows"))
        .map(|(_, c)| *c);
...
fn pql_group_by_missing_field_handled() {
    ...
    let mut batch = WriteBatch::new();
    batch.upsert_entity(entity_with_props("host", "Host", "with-region",    &[("region", Value::from("us-east-1"))]));
    ...
    assert_eq!(groups.len(), 2, "with-region and without-region are two distinct groups");
}
...
fn pql_group_by_total_count_equals_entity_count() {
    ...
    for (i, env) in ["prod", "prod", "dev", "staging"].iter().enumerate() {
        batch.upsert_entity(entity_with_props("host", "Host", &format!("h{i}"), &[("env", Value::from(*env))]));
    }
    engine.write(batch).unwrap();
    ...
    assert_eq!(total, 4, "sum of group counts must equal total entity count");
